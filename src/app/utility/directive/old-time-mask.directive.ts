...
@Directive({
  selector: '[appTimeMaskOld]',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => OldTimeMaskDirective),
      multi: true,
    },
    {
...
export class OldTimeMaskDirective implements OnInit, ControlValueAccessor, Validator {
  /** implements ControlValueAccessorInterface */
  _onChange: ((_: Date) => void) | undefined;
  /** implements ControlValueAccessorInterface */
  _touched: (() => void) | undefined;
  ...
  private _setHours(hours: string, minutes: string, key: string) {
    ...
    if (firstDigit === '-' || this._fieldJustGotFocus) {
      ...
      sendCaretToMinutes = Number(key) > 99;
    } else {
